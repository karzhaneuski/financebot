# FinanceBot — AGENTS.md

## Project overview
Telegram bot for personal finance tracking via receipt photos.
User sends a photo of a receipt → Codex Vision parses it → data is stored in PostgreSQL → user can query statistics.
...
- **AI parsing**: Anthropic Codex Vision API (`Codex-sonnet-4-20250514`)
...
│   ├── services/
│   │   ├── __init__.py
│   │   ├── vision.py        # Codex Vision API call
...
│   └── utils/
│       ├── __init__.py
│       ├── validators.py    # validate parsed JSON from Codex
...
- Log all Codex Vision API errors to stderr with full traceback
